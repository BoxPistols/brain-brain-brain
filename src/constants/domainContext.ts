...
■ 求職者ファネル（各段階の典型転換率）:
${SEEKER_FUNNEL.map(s => `  ${s.stage}: ${s.cr}`).join('\n')}

■ 企業クライアントファネル:
${CLIENT_FUNNEL.map(s => `  ${s.stage}: ${s.cr}`).join('\n')}
...
- keyIssue に最重要イシューを1文で特定`.trim();
  }
  return `[HR文脈] 成功報酬型（年収×30-35%）。CA=求職者担当/RA=企業担当。求職者ファネル: 認知→登録→面談→紹介→応募→内定承諾。企業: リード→契約→受注→リピート。課題: エージェント不信・情報非対称性・CA属人化・認知ギャップ・早期退職返金。マッチング3軸: スキル・カルチャー・期待値。funnelStageにボトルネック段階、keyIssueに最重要イシューを出力。`;
}
