    ...
    return (
        <div className='space-y-2'>
            {category.items.map((item, i) => (
                <div key={i} className={`rounded-lg border ${T.cardFlat.includes('border') ? '' : 'border-slate-200 dark:border-slate-700/60'} overflow-hidden`}>
                    <button
                    ...
                    >
                        <span>{item.q}</span>
                        <ChevronDown className={`w-3.5 h-3.5 shrink-0 transition-transform duration-200 ${T.t3} ${openIdx === i ? 'rotate-180' : ''}`} />
                    </button>
                    <div
