...
import { Wand2, CalendarRange, CalendarCheck, SlidersHorizontal, Layers, Plus } from 'lucide-react';
import { BrainstormForm } from '../../types';
import { TYPES, ISSUE_TPL, DEPTH } from '../../constants/prompts';
...
export const ProjectForm: React.FC<ProjectFormProps> = ({ form, setForm, dep, setDep }) => {
  ...
  const setIssue = (i: number, k: keyof BrainstormForm['issues'][number], v: string) => 
    setForm(p => { const is = [...p.issues]; is[i] = { ...is[i], [k]: v }; return { ...p, issues: is }; });
    
  const addIssue = (t = '') => 
    setForm(p => ({ ...p, issues: [...p.issues, { text: t, detail: '', sub: [] }] }));
    
  const rmIssue = (i: number) => 
    setForm(p => ({ ...p, issues: p.issues.filter((_, j) => j !== i) }));
    
  const addSub = (i: number) => 
    setForm(p => { const is = [...p.issues]; is[i] = { ...is[i], sub: [...(is[i].sub || []), ''] }; return { ...p, issues: is }; });
    
  const setSub = (i: number, si: number, v: string) => 
    setForm(p => { const is = [...p.issues]; const s = [...(is[i].sub || [])]; s[si] = v; is[i] = { ...is[i], sub: s }; return { ...p, issues: is }; });
    
  const rmSub = (i: number, si: number) => 
    setForm(p => { const is = [...p.issues]; is[i] = { ...is[i], sub: (is[i].sub || []).filter((_, j) => j !== si) }; return { ...p, issues: is }; });
      ...
      <div className="mb-3">
        ...
        <div className="flex items-center gap-2">
          <div className={`flex shrink-0 rounded-lg overflow-hidden border ${T.div}`}>
            {(Object.entries({ period: { l: '期間', I: CalendarRange }, deadline: { l: '期日', I: CalendarCheck }}) as [('period' | 'deadline'), { l: string; I: React.ElementType }][]).map(([m, { l, I }]) => (
      ...
      <div className="mb-3">
        ...
        <div className="space-y-1.5">
          {form.issues.map((iss, i) => (
            <IssueRow key={i} issue={iss} idx={i} onChange={setIssue} onRemove={rmIssue} onAddSub={addSub} onSubChange={setSub} onRemoveSub={rmSub} />
          ))}
        </div>
