    ...
    <div className='grid grid-cols-2 md:grid-cols-3 gap-1.5 mb-3'>
        {clusters.map(c => {
            ...
            return (
                ...
                >
                    <div className={`flex items-center gap-1.5 text-xs font-semibold ${active ? 'text-blue-700 dark:text-blue-300' : T.t1}`}>
                        {ICONS[c.id]}
                        {c.label}
                    </div>
                    <div className={`text-[10px] mt-0.5 ${active ? 'text-blue-600/70 dark:text-blue-400/70' : T.t3}`}>
                        {c.sublabel}
                    </div>
                </button>
            )
