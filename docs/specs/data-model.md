...
ブレインストーミングモードを表す文字列リテラル型。トップレベルの切替UIで選択。

| 値 | ラベル | 対象ユーザー |
|---|--------|------------|
| `strategy` | 戦略モード | 経営・マーケ・営業 |
| `player` | プレイヤーモード | エンジニア・テックリード |
| `designer` | デザイナーモード | デザイナー・UXリサーチャー |

## SessionType

セッション種別を表す文字列リテラル型。各モードに紐づく。
...
| 値 | ラベル | 用途 |
|---|--------|------|
| `dev-org` | 開発組織・フロー設計 | チーム編成・CI/CD・スプリント設計・技術負債管理 |
| `design-org` | デザイン組織・プロセス設計 | DesignOps・レビューフロー・デザイナー×エンジニア協業 |
| `task-flow` | タスクフロー・実行設計 | ボトルネック特定・WIP制限・依存関係可視化 |
| `personal-mission` | 個人ミッション・アクション設計 | OKR分解・工数見積もり・ブロッカー排除 |
| `arch-design` | アーキテクチャ設計・技術戦略 | DDD・Clean Arch・マイクロサービス設計 |
| `test-quality` | テスト戦略・品質設計 | テストピラミッド・TDD・品質ゲート |
| `devops-delivery` | DevOps・デリバリー基盤 | CI/CD・SRE・オブザーバビリティ |
| `frontend-eng` | フロントエンド・UI基盤設計 | コンポーネント設計・Core Web Vitals |
...
| `ux-research` | UXリサーチ・ユーザビリティ評価 | ニールセンヒューリスティクス・ユーザーテスト |
| `accessibility` | アクセシビリティ・インクルーシブデザイン | WCAG準拠・支援技術対応 |
| `interaction-design` | インタラクションデザイン | マイクロインタラクション・モーション |
| `design-handoff` | デザイン-エンジニアリング協業 | ハンドオフ・トークン同期・デザインQA |
